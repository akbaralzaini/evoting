...
public class PasanganAdapter extends RecyclerView.Adapter<PasanganAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        String url = "https://voting.pkitapp.com/public/uploads/kandidat/"+mPasanganList.get(position).getId_ketua()+"/"+mPasanganList.get(position).getFoto_ketua();
        Picasso.get().load(url).resize(500,500).into(holder.mFotoKetua);

        String urlw = "https://voting.pkitapp.com/public/uploads/kandidat/"+mPasanganList.get(position).getId_wakil()+"/"+mPasanganList.get(position).getFoto_wakil();
